...
**Student Answer:** The top 5 PCs seem to explain a lot more variance than the rest of the PCs, thus 5 PCs would be a reasonable choice, although we might also want to take into account of the performance of clustering.
```{r}
...
We should NOT scale for clustering purpose. Since from the two plots below, the unscaled PCs can produce much better separation of different cancer types (especially the difference between Basal and Her2 Type).
...
**Student Answer:** K = 4 is a reasonable number for cluster according to the elbow rule (although in our particular case the location of the "elbow" is a bit ambiguous).
...
```{r}
pca_ct_kmeans <- kmeans(x = pca_ct$x[,1:4], centers = 4, nstart = 100)
...
plot_1 <-
  ...
  ggplot() + 
  geom_point(aes(x = x, y = y, col = label, shape = cluster)) + 
  labs(title = "Kmeans with 4PCs", x = 'PC 1', y = 'PC 2')

brca_sub_kmeans <- kmeans(x = brca_sub, centers = 4, nstart = 10)
